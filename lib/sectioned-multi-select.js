...
class SectionedMultiSelect extends PureComponent {
  ...
  _renderItemFlatList = ({ item, index }) => {
    let { styles, colors } = this.state

    const { searchTerm } = this.state
    if (this.props.stripedBG) {
      colors.itemBackground = this.props.stripedBG[index % 2]
    }
    if (this.props.stripedText) {
      colors.text = this.props.stripedText[index % 2]
    }
    return (
      <View>
        <RowItem
